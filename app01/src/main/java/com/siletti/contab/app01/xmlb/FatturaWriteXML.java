...
//				BigDecimal totaleImponibile = row.getBigDecimal("TotaleImponibile"); 
				...
				// ATTENZIONE Attuale fatturazione ha sempre UN UNICA aliquota Iva da applicare
...
//				String descrizioneIva = row.getString("rDescrizione1"); 
				...
				FatturaElettronicaBodyType myFatturaElettronicaBody = myFatturaElettronica
						.addNewFatturaElettronicaBody();
...
//	    		Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
				...
				if (trasporto.compareTo(BigDecimal.ZERO) == 1) {
					...
					if (aliquotaIva.compareTo(BigDecimal.ZERO) == 0)
						dettaglioLinee.setNatura(naturaIva);
				}
				...
				// Salva file finale
				String fileNameNew = fatture.getParent() + java.io.File.separatorChar
						+ calDataDocumento.get(Calendar.YEAR) + "-" + String.format("%04d", NumeroDocumento) + "-"
						+ cliente + ".xml";
